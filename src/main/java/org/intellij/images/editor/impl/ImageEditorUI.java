  ...
  ImageEditorUI(@Nullable ImageEditor editor) {
    ...
    copyPasteSupport = editor != null ? new CopyPasteDelegator(editor.getProject(), this) {
      ...
      protected PsiElement[] getSelectedElements() {
        ...
        return consulo.ide.impl.idea.util.ObjectUtils.notNull(dataContext.getData(LangDataKeys.PSI_ELEMENT_ARRAY), PsiElement.EMPTY_ARRAY);
      }
    } : null;
      ...
      public Double get() {
        ...
        if (IfsUtil.isSVG(file)) {
          try {
            ...
            return Math.max(1, SVGLoader.getMaxZoomFactor(url, new ByteArrayInputStream(file.contentsToByteArray()), JBUI.ScaleContext.create(editor.getComponent())));
          } catch (Throwable t) {
